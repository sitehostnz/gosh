package dns

import (
	"context"
	"io"
	"net/http"
	"net/http/httptest"
	"testing"

	"github.com/sitehostnz/gosh/pkg/api"
)

func TestListIPs_Success(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/dns/list_ips.json" {
			t.Errorf("path = %q, want /dns/list_ips.json", r.URL.Path)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{
			"status": true,
			"msg": "Successful",
			"return": {
				"203.0.113.10": {
					"ip_addr": "203.0.113.10",
					"netmask": "255.255.255.0",
					"prefix": "32",
					"reserved": "0",
					"rdns": "rdns.203.0.113.10.example",
					"addr_family": "4",
					"date_allocated": "2026-03-01 12:00:00",
					"server_id": "12345",
					"name": "ch-example",
					"label": "example",
					"is_primary": "1",
					"ip_type": "1"
				}
			}
		}`)
	}))
	defer server.Close()

	c, err := api.New("k", "1", api.SetBaseURL(server.URL))
	if err != nil {
		t.Fatalf("api.New: %v", err)
	}

	got, err := New(c).ListIPs(context.Background())
	if err != nil {
		t.Fatalf("ListIPs: %v", err)
	}

	if len(got.Return) != 1 {
		t.Fatalf("len(Return) = %d, want 1", len(got.Return))
	}
	ip, ok := got.Return["203.0.113.10"]
	if !ok {
		t.Fatal("expected 203.0.113.10 key in Return map")
	}
	if ip.AddrFamily != "4" {
		t.Errorf("AddrFamily = %q, want 4", ip.AddrFamily)
	}
	if ip.ServerID != "12345" {
		t.Errorf("ServerID = %q, want 12345", ip.ServerID)
	}
	if ip.IsPrimary != "1" {
		t.Errorf("IsPrimary = %q, want 1", ip.IsPrimary)
	}
}
